...
func GetUserInfo(userId string) common.User {
	...
	var res common.User
	db.QueryRow(sqlStr).Scan(&res.Id, &res.Username, &res.Password, &res.Gender, &res.RegisterTime, &res.CountFollow, &res.CountFollowed, &res.Description, &res.CountMoment, &res.CountCreatePlaylist, &res.CountLikePlaylist)
	res.Password = " "
	return res
}
